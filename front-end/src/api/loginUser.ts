...
export const loginUser = async (credentials: LoginCredentials): Promise<LoginResponse> => {
  try {
    ...
    console.log('👤 Usuário encontrado no cache:', {
      ...
      telefone: cachedUser.telefone,
      cpf: cachedUser.cpf
    });
    
    // Valida credenciais baseado no método de login
    let credentialValid = false;
    
    if (credentials.email) {
      credentialValid = cachedUser.email === credentials.email;
      console.log('🔍 Validando por email:', credentialValid);
    } else if (credentials.telefone) {
      credentialValid = cachedUser.telefone === credentials.telefone;
      console.log('🔍 Validando por telefone:', credentialValid);
    } else if (credentials.cpf) {
      credentialValid = cachedUser.cpf === credentials.cpf;
      console.log('🔍 Validando por CPF:', credentialValid);
    }
    ...
    // Valida senha (em produção, usar hash)
    if (cachedUser.senha !== credentials.senha) {
