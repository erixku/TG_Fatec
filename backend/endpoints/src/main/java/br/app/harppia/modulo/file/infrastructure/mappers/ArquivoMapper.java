...
public abstract class ArquivoMapper {
    ...
    @Mapping(source = "bucket", target = "bucket", qualifiedByName = "bucketDtoParaBucketEntity")
    ...
    @Named("bucketDtoParaBucketEntity")
    protected BucketEntity mapBucketDtoToBucketEntity(BucketCadastroDTO bucketDto) {
    	
        if (bucketDto == null || bucketDto.nome() == null) 
            return null;

        BucketBasicInfo bucketInfo = buscarBucketUseCase.findByNome(bucketDto.nome());
